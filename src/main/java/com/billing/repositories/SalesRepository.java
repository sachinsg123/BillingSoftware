...
public interface SalesRepository extends JpaRepository<Sales, Integer> {
	...
	@Query("SELECT c FROM Sales c JOIN c.user u WHERE c.status = 'Active' AND c.salesType = 'Return' AND u.id = :id")
	List<Sales> showAllActiveSalesReturn(@Param("id") Integer id);
	...
	@Query("SELECT c FROM Sales c JOIN c.user u WHERE c.status = 'Active' AND c.paymentMode = 'cash' AND c.salesType = 'Sale' AND u.id = :id")
	...
	@Query("SELECT c FROM Sales c JOIN c.user u WHERE c.status = 'Active' AND c.paymentMode = 'online' AND c.salesType = 'Sale' AND u.id = :id")
	Page<Sales> showAllOnlinePayment(@Param("id") Integer id, Pageable pageable);
	
	@Query("SELECT c FROM Sales c JOIN c.user u WHERE c.status = 'Active' AND c.paymentMode = 'cheque' AND c.salesType = 'Sale' AND u.id = :id")
